...
def test_point_groups_no_duplicate_labels():
    names = [pg.label.name for pg in POINT_GROUPS]
...
def test_point_groups_orders_positive():
    ...
    for pg in POINT_GROUPS:
        if not pg.label.is_linear():
            assert pg.order > 0, f"{pg.label.name} has order 0"
...
def test_c1_has_order_1():
    c1 = next(pg for pg in POINT_GROUPS if pg.label.name == "C1")
    assert c1.order == 1


def test_oh_has_order_48():
    oh = next(pg for pg in POINT_GROUPS if pg.label.name == "Oh")
    assert oh.order == 48


def test_ih_has_order_120():
    ih = next(pg for pg in POINT_GROUPS if pg.label.name == "Ih")
    assert ih.order == 120


def test_td_has_order_24():
    td = next(pg for pg in POINT_GROUPS if pg.label.name == "Td")
    assert td.order == 24
...
def test_ci_requires_inversion():
    import numpy as np
    ci = next(pg for pg in POINT_GROUPS if pg.label.name == "Ci")
    # without inversion → -1
...
def test_cs_requires_reflection():
    import numpy as np
    cs = next(pg for pg in POINT_GROUPS if pg.label.name == "Cs")
...
def test_c2v_requires_c2_and_two_reflections():
    import numpy as np
    c2v = next(pg for pg in POINT_GROUPS if pg.label.name == "C2v")
    # missing everything
